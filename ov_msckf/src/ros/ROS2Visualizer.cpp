...
void ROS2Visualizer::publish_features() {
  ...
  rclcpp::Time feature_stamp = _node->now();
  if (_app->initialized()) {
    feature_stamp = ROSVisualizerHelper::get_time_from_seconds(_app->get_state()->_timestamp);
  }

  // Get our good MSCKF features
  std::vector<Eigen::Vector3d> feats_msckf = _app->get_good_features_MSCKF();
  sensor_msgs::msg::PointCloud2 cloud = ROSVisualizerHelper::get_ros_pointcloud(_node, feats_msckf, feature_stamp);
  pub_points_msckf->publish(cloud);

  // Get our good SLAM features
  std::vector<Eigen::Vector3d> feats_slam = _app->get_features_SLAM();
  sensor_msgs::msg::PointCloud2 cloud_SLAM = ROSVisualizerHelper::get_ros_pointcloud(_node, feats_slam, feature_stamp);
  ...
  sensor_msgs::msg::PointCloud2 cloud_ARUCO = ROSVisualizerHelper::get_ros_pointcloud(_node, feats_aruco, feature_stamp);
  ...
  sensor_msgs::msg::PointCloud2 cloud_SIM = ROSVisualizerHelper::get_ros_pointcloud(_node, feats_sim, feature_stamp);
