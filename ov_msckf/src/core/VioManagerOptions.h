  ...
  void print_and_load_state(const std::shared_ptr<ov_core::YamlParser> &parser = nullptr) {
    if (parser != nullptr) {
      ...
      for (int i = 0; i < state_options.num_cameras; i++) {
        ...
        // Optional external camera_models calibration file for runtime-only camera model support.
        ...
        if (!calib_file.empty()) {
          if (state_options.do_calib_camera_intrinsics) {
            PRINT_ERROR(RED "camera_models runtime integration currently requires calib_cam_intrinsics: false\n" RESET);
